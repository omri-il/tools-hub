...
10. **מקצר קישורים מהיר (TinyURL)** *(public)* → `tinyurl/` (in-repo static page). URL → plain `tinyurl.com/xxx` via the now-public `go-shortener` `/api/tinyurl` (CORS-locked to the tools origin; no password). Split out from #6 so anyone can use it.
