...
6. **מקצר קישורים** → `shortener/` (in-repo static page). Management UI for Omri's own URL shortener. The page is static (GitHub Pages) and calls the **`go-shortener`** Flask service at `https://go.omri-iram.co.il/api/*` over CORS — create short links (optional custom alias), list them with click counts, delete. Password-gated; the admin password is stored in `localStorage` only (`go_shortener_pw`) and sent as `X-Admin-Password`. The redirect backend + DB live in the separate `go-shortener` repo on the VPS (not here). `noindex`.
